...
def auditoria_Lighthouse(url, mode):
    ...
    return os.path.abspath(final_HTML)  # Retorna la ruta absoluta del archivo HTML
...
# Función para capturar una imagen del archivo HTML generado
def capturar_imagen(html_path, image_path):
...
def urls_Lighthouse(url):
    ...
    if codigo is None or codigo != 200:
        # Si la URL no es válida, registrar el error en el Excel y no ejecutar las pruebas
        actualizar_excel(url, {'performance': None, 'accessibility': None, 'seo': None}, {'performance': None, 'accessibility': None, 'seo': None}, codigo, descripcion_codigo)
        return {
            'url': url,
            'total_test': None
        }
    ...
    if reporte_MOBILE:
        ...
        # Capturar imagen del reporte
        capturar_imagen(reporte_MOBILE, f'mobile_{os.path.basename(reporte_MOBILE)}.png')
    else:
    ...
    if reporte_DESKTOP:
        ...
        # Capturar imagen del reporte
        capturar_imagen(reporte_DESKTOP, f'desktop_{os.path.basename(reporte_DESKTOP)}.png')
    else:
